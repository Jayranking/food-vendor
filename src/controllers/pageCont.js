...
module.exports = {
  index: async (req, res) => {
    ...
    if (search) {
      // Create filter conditions for searching by name, amount, or vendor name
      filter = {
        $or: [
    ...
    try {
      const _listedFood = await Food.find(filter).populate("vendor");
  ...
  makePayment: async (req, res) => {
    ...
    const cust_emailReg =
      /^[a-z0-9]([a-z0-9_\.\-])*\@(([a-z0-9])+(\-[a-z0-9]+)*\.)+([a-z0-9]{2,4})/;
